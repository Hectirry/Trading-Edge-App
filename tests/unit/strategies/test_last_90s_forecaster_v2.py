...
def test_edge_below_threshold_skips() -> None:
    ...
    d = _strat(model=model, macro=_StubMacro(_macro("range"))).should_enter(_ctx(implied=0.48))
    assert d.action is Action.SKIP
...
def test_range_regime_allows_both_directions() -> None:
    model = _StubModel(p=0.70)
    d = _strat(model=model, macro=_StubMacro(_macro("range"))).should_enter(_ctx(implied=0.40))
    assert d.action is Action.ENTER
...
def test_spread_too_wide_blocks_before_model() -> None:
    model = _StubModel(p=0.70)
    d = _strat(model=model, macro=_StubMacro(_macro())).should_enter(_ctx(spread_bps=300.0))
    assert d.action is Action.SKIP
...
def test_outside_entry_window_skips_before_model() -> None:
    model = _StubModel(p=0.70)
    d = _strat(model=model, macro=_StubMacro(_macro())).should_enter(_ctx(t_in_window=100.0))
    assert d.action is Action.SKIP
